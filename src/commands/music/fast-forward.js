...
class FastForwardCommand extends Command {
  ...
  async do(message, [rewind]) {
    try {
      ...
      if (!player.queue.current.isSeekable) return message.channel.send("", { embed : this.client.BaseEmbed(message).setColor(this.client.color("red")).setDescription("`🚫` You can't rewind a live video.") });

      const time = this.read24hrFormat((rewind) ? rewind : "10");
      if (time + player.position >= player.queue.current.duration) {
        return message.channel.send("", { embed : this.client.BaseEmbed(message).setColor(this.client.color("red")).setDescription(`\`🚫\` The song is only ${new Date(player.queue.current.duration).toISOString().slice(14, 19)}.`) });
      } else {
        player.seek(player.position - time);
        return message.channel.send("", { embed : this.client.BaseEmbed(message).setColor(this.client.color("orange")).setDescription(`\`⏩\` Fast-Forward: \`${new Date(player.position).toISOString().slice(14, 19)}\` to **${this.getReadableTime(time)}**`) });
      }
    } catch (e) {
