    ...
    try {
      ...
      if (!guild) {
        ...
        if (player && player.paused) {
          player.pause(false);
          return message.channel.send("", { embed : this.client.BaseEmbed(message).setDescription("`⏯` Resumed the player.") });
        }
        if (!query.length > 0) {
          if (message.attachments.size > 0) {
            ...
            if (!query) return message.channel.send("", { embed : this.client.BaseEmbed(message).setColor(this.client.color("red")).setDescription(`\`❌\` There was an error while playing: \`IMAGE/INVALID_FILE\``) });
          } else {
            query = await this.client.nekoyasui.prompt.reply(message.channel, "What song, do you want me to play?", { userID: message.author.id });
          }
        }
        try {
          res = await this.client.erela.search(query.join(" ").trim(), message.author);
        ...
        case "SEARCH_RESULT": {
          ...
          if (!player.playing && !player.paused && !player.queue.size) player.play();
          return message.channel.send("", { embed : this.client.BaseEmbed(message).setDescription(`\`🎵\` Added Track: **[${track.title}](${track.uri}).**`) });
        }
        }
      }
