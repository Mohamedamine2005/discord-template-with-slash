    ...
    try {
      ...
      if (!guild) {
        ...
        try {
          ...
          console.log(e.stack);
          return message.channel.send("", { embed : this.client.BaseEmbed(message).setColor(this.client.color("red")).setDescription(`\`❌\` There was an error while searching: ${e.message}`) });
        }
        if (!res.length > 0) return message.channel.send("", { embed : this.client.BaseEmbed(message).setColor(this.client.color("red")).setDescription("`🚫` No radio found with that name, maybe try searching for something that exists.") });
        ...
        switch (res.loadType) {
          ...
          default:
            ...
            return message.channel.send("", { embed : this.client.BaseEmbed(message).setDescription(`\`🎵\` Added Track: **[${res.tracks[0].title}](${res.tracks[0].uri}).**`) });
        }
      }
      ...
      switch (res.loadType) {
        ...
        default:
          ...
          return message.channel.send("", { embed : this.client.BaseEmbed(message).setDescription(`\`🎵\` Added Track: **[[${res.tracks[0].title}](${res.tracks[0].uri})](${res.tracks[0].uri}).**`) });
      }
