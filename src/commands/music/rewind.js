  ...
  async do(message, [rewind]) {
    try {
      ...
      if (time + player.position <= 0) {
        return message.channel.send("", { embed : this.client.BaseEmbed(message).setColor(this.client.color("red")).setDescription("`🚫` A song can not be less than 0 seconds long") });
      } else {
        player.seek(player.position - time);
        return message.channel.send("", { embed : this.client.BaseEmbed(message).setColor(this.client.color("orange")).setDescription(`\`⏪\` Rewind: \`${new Date(player.position).toISOString().slice(14, 19)}\` to **${this.getReadableTime(time)}**`) });
      }
    } catch (e) {
