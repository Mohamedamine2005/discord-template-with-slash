...
class SeekCommand extends Command {
  ...
  async do(message, [rewind]) {
    try {
      ...
      if (!player.queue.current.isSeekable) return message.channel.send("", { embed : this.client.BaseEmbed(message).setColor(this.client.color("red")).setDescription("`🚫` You can't seek a live video.") });

      const time = this.read24hrFormat((rewind) ? rewind : "10");
      if (time > player.queue.current.duration) {
        return message.channel.send("", { embed : this.client.BaseEmbed(message).setColor(this.client.color("red")).setDescription(`\`🚫\` Less than ${player.queue.current.duration}`) });
      } else {
        player.seek(time);
        return message.channel.send("", { embed : this.client.BaseEmbed(message).setColor(this.client.color("orange")).setDescription(`\`🎵\` Seek: \`${new Date(player.position).toISOString().slice(14, 19)}\` to **${this.getReadableTime(time)}**`) });
      }
    } catch (e) {
