...
const Callback = async (res, data) => {
  ...
  let extras = { member, channel, guild, user: res.user || null };
  let interaction = new method(res.client, data, extras);
  return interaction;
};
...
class method {
  ...
  async reply(res, options = {}) {
    ...
    return this.client.api.webhooks(this.client.user.id, this.token).post({ data: {
      type: type,
      content: res,
      embeds: embeds,
      tts: tts,
  ...
  async edit(content = "", options = {}) {
    ...
    if (embed) embeds.push(embed);
    return this.client.api.webhooks(this.client.user.id, this.token).messages(this.messageRefID).patch({ data:{
      type: type,
      content: content,
      embeds: embeds,
      tts: tts,
